...
const VALID_FILTERS = new Set(
  ROADMAP_CATEGORY_FILTERS.map((filter) => filter.id),
);

function parseCategoryParam(value: string | null): RoadmapCategoryFilter {
  if (!value || !VALID_FILTERS.has(value as RoadmapCategoryFilter)) {
    return "all";
  }
  return value as RoadmapCategoryFilter;
}
...
export function RoadmapCatalog({
  ...
  const [category, setCategory] = useState<RoadmapCategoryFilter>(urlCategory);
  ...
  return (
    <section
      id="roadmaps"
      className="page-gutter-x site-container scroll-mt-20 py-12 min-[390px]:py-14 min-[428px]:py-16 sm:py-16 xl:py-20"
    >
      {showHeader && (
        <AnimateIn>
            ...
            <div>
              ...
              <h2 className="mt-2 text-2xl font-bold tracking-tight text-zinc-900 min-[390px]:text-[1.75rem] min-[428px]:text-3xl dark:text-zinc-50">
                {category === "all"
                  ...
                  : filtered.length === 1
                    ? "1 roadmap in this category"
                    : `${filtered.length} roadmaps in this category`}
              </h2>
            </div>
          </div>
        </AnimateIn>
      ...
      {showFilters && collections.length > 0 && (
        <CategoryChips
          ...
          className={showHeader ? "mt-8" : "mt-0"}
        />
      )}
      ...
      {standalonePages.length > 0 && category === "all" && (
        <div className="mt-14">
